...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            // 绑定学生
            bindmodule: function(pid,mid) {
            	var url = $.common.isEmpty(pid) ? $.table._option.bindModUrl : $.table._option.bindModUrl.replace("{pid}", pid);
            	url = $.common.isEmpty(mid) ? $.table._option.bindModUrl : url.replace("{mid}", mid);
                $.modal.openFull("编辑模块" + $.table._option.modalName, url);
            },


            // 新增二参数
            addtwo: function(pid,mid) {
            	var url = $.common.isEmpty(pid) ? $.table._option.createUrl : $.table._option.createUrl.replace("{pid}", pid);
            	url = $.common.isEmpty(mid) ? $.table._option.createUrl : url.replace("{mid}", mid);
                $.modal.open("添加" + $.table._option.modalName, url);
            },
            
         // 查看绑定学生
            lookstu: function(id) {
            	var url = $.common.isEmpty(id) ? $.table._option.lookStuUrl : $.table._option.lookStuUrl.replace("{id}", id);
                $.modal.openFull("编辑模块" + $.table._option.modalName, url);
            },
            ...
            // 修改2信息
            edittwo: function(pid,mid) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(pid)) {
            	    url = $.table._option.updateUrl.replace("{pid}", pid);
        ...
        tree: {
        	...
        	// 获取当前被勾选集合子
        	getCheckedNodesS: function(column) {
        		var _column = $.common.isEmpty(column) ? "id" : column;
